...
contract LendingPool {
    ...
    struct Loan {
        ...
        bool insuranceTaken; // Whether insurance was taken for this loan
    }
    ...
    function _createLoan(
        ...
        uint256 durationMonths,
        uint256 annualInterestRate,
        uint256 downPaymentPercentage // New parameter
    ) internal returns (bytes32) {
        // Calculate dynamic borrower deposit and lender principal
        uint256 borrowerDeposit = (totalAmount * downPaymentPercentage) / 10000;
        ...
        newLoan.monthlyPayment =
            (lenderPrincipal * monthlyRate) /
            (1e18 - (1e18 / (1e18 + monthlyRate)) ** durationMonths);
    ...
    function _createAmortizationSchedule(
        ...
        for (uint256 m = 0; m < months; m++) {
            ...
            loan.amortizationSchedule.push(
                Installment({
    ...
    function debugUnstakeCalc(
        bytes32 loanId,
        uint256 principalRepaid
    ) external view returns (uint256 proportionRepaid, uint256 cbBtcToUnstake) {
        Loan storage loan = loans[loanId];
    ...
    function depositToPool(uint256 amount, bool reinvest) external {
        ...
        emit Deposit(msg.sender, amount, reinvest);
    }
     ...
     * - Lender principal (80%)
     */
    function computeLoanParts(
        uint256 totalAmount
    ) public pure returns (uint256 borrowerDeposit, uint256 lenderPrincipal) {
        borrowerDeposit = (totalAmount * 20) / 100;
        lenderPrincipal = totalAmount - borrowerDeposit;
    }

    function getDownPaymentBasisPoints(
        uint256 totalAmount,
        uint256 borrowerDeposit
    ) external pure returns (uint256 downPaymentBps, bool isValidDownPayment) {
        require(totalAmount > 0, "Total amount must be greater than 0");
        require(borrowerDeposit <= totalAmount, "Borrower deposit cannot exceed total amount");

        // Calculate the down payment percentage in basis points
        // Formula: (borrowerDeposit * 10000) / totalAmount
        downPaymentBps = (borrowerDeposit * 10000) / totalAmount;

        // Check if the calculated percentage is within valid range (20% to 50%)
        isValidDownPayment = (downPaymentBps >= 2000 && downPaymentBps <= 5000);

        return (downPaymentBps, isValidDownPayment);
    }
    ...
    {
        isValidDownPayment = (downPaymentPercentage >= 2000 && downPaymentPercentage <= 5000);

        if (isValidDownPayment) {
            borrowerDeposit = (totalAmount * downPaymentPercentage) / 10000;
            lenderPrincipal = totalAmount - borrowerDeposit;
        } else {
            borrowerDeposit = 0;
            lenderPrincipal = 0;
        ...
        return (borrowerDeposit, lenderPrincipal, isValidDownPayment);
    }
}
