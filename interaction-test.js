...
async function interactor(
  ...
  const mockDeployer = await ethers.getContractAt("MockDeployer", mockDeployerAddress);
  ...
  console.log(`Start time: ${new Date(loanDetails.startTime.toNumber() * 1000).toLocaleString()}`);
  ...
  for (let i = 0; i < Math.min(3, schedule.length); i++) {
    ...
    console.log(`  Due date: ${new Date(payment.dueTimestamp.toNumber() * 1000).toLocaleString()}`);
