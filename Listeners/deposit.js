...
const recordDeposit = async () => {
  try {
    ...
    console.log(`Checking for deposit events from block ${blockNumber - 100} to ${blockNumber}`);
    ...
    for (const event of depositEvents) {
      ...
      console.log(`Processing deposit from ${lender} of amount ${amount.toString()}`);
      ...
      const timestamp = txBlock ? txBlock.timestamp : Math.floor(Date.now() / 1000);
      ...
      console.log(`Successfully added lending with ID ${lending._id} for user ${user._id}`);
    }
  } catch (error) {
